...
def process_all_datasets(base_dir, pipeline_path):
    ...
    drugs = [f for f in Path(base_dir).glob('*')]
    datasets = {str(d): [str(fp).split('/')[-1] for fp in list(d.glob('*'))] for d in drugs}
    # datasets = {
    #     # 'egfr': ['ko', 'control'],
    #     'afatinib': [
    #         'compound-003_Plate10_compound-003_10_AA14_conc10.0'
    #     ]  # Modify it based on actual drug dataset names - i'm not familiar with the names
    # }
    # import ipdb; ipdb.set_trace()
    for study_type, conditions in datasets.items():
        for condition in conditions:
            input_dir = os.path.join(base_dir, study_type, condition)
            ...
            try:
                ...
                print(f"Successfully completed analysis for {study_type}/{condition}")
            except Exception as e:
...
if __name__ == "__main__":
    # config
    BASE_DIR = "/mnt/data/egfr-images3/EGFR_inhibitors_and_controls"  # modify this path
    PIPELINE_PATH = "/home/ubuntu/morphologic/pipelines/rxrx3.cppipe"  # modify this path
